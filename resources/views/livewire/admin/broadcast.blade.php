<div>
    ...
    <div class="admin-card" style="max-width: 800px;">
        ...
        <form wire:submit.prevent="send">
            <div style="display: flex; flex-direction: column; gap: 1.5rem;">
                ...
                <!-- Image Upload -->
                <div>
                    <label class="form-label">Promotion Image (Optional)</label>
                    <div style="display: flex; flex-direction: column; gap: 1rem;">
                        <input type="file" wire:model="image" class="form-input" accept="image/*" id="image-upload">
                        <div wire:loading wire:target="image" style="color: #FF7A00; font-size: 0.8rem;">Uploading...</div>
                        @error('image') <span style="color: #EF4444; font-size: 0.8rem;">{{ $message }}</span> @enderror
                        
                        @if ($image)
                            <div style="position: relative; width: fit-content;">
                                <img src="{{ $image->temporaryUrl() }}" style="width: 150px; height: 100px; object-fit: cover; border-radius: 8px;">
                                <button type="button" wire:click="$set('image', null)" style="position: absolute; top: -10px; right: -10px; background: #EF4444; color: white; border: none; border-radius: 50%; width: 24px; height: 24px; cursor: pointer; display: flex; align-items: center; justify-content: center;">&times;</button>
                            </div>
                        @endif
                    </div>
                </div>
            ...
            <div style="margin-top: 2rem; padding-top: 1.5rem; border-top: 1px solid rgba(255,255,255,0.05); display: flex; justify-content: flex-end;">
                <button type="submit" class="btn-primary" wire:loading.attr="disabled">
                    ...
                    <span wire:loading wire:target="send">Sending to users...</span>
                </button>
            </div>
        </form>
    </div>
    ...
    <div class="admin-card" style="max-width: 800px; margin-top: 2rem; background: rgba(0,0,0,0.2);">
        ...
        <div style="background: white; color: #1a1a1a; padding: 2rem; border-radius: 12px;">
            ...
            @if ($image)
                <img src="{{ $image->temporaryUrl() }}" style="width: 100%; height: 250px; object-fit: cover; border-radius: 12px; margin-bottom: 1.5rem;">
            @endif
